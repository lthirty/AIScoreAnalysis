...
- **异常恢复**：增强分析若遇到异常，会自动回退到规则报告；错误会保留在日志里，便于后续直接排查。
- **自愈模式**：增强分析会先正常生成，失败后自动用修复提示再试一次，只有两次都失败才回退规则报告。
...
| 版本 | 日期 | 修改内容 |
|------|------|----------|
| 3.0.17 | 2026-05-16 | 增强分析升级为服务端两段式自愈：先正常生成，失败后自动用修复提示再试一次，最后才回退规则报告；失败日志保留完整异常和截断后的原始输出；版本记录同步升级 |
| 3.0.16 | 2026-05-16 | 增强分析异常继续自动回退到规则报告，但不再提示客户返回重进；后端增强分析异常统一记录完整日志，便于后续直接排查；版本记录同步升级 |
