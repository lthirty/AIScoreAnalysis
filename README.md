...
### 当前报告页行为
- **AI 基础分析**：展示总分、平均分、各科对比、优势/薄弱科目、优先提升和下次目标。
...
- **缺少数据处理**：开始增强分析前会检查缺少学科并弹窗确认；缺少数据的学科会在结果里直接显示“由于缺少数据，该科目无法进行深入分析。”。
...
| 版本 | 日期 | 修改内容 |
|------|------|----------|
| 5.0.0 | 2026-05-17 | 升级为大版本：各科趋势深度分析与增强分析调试信息默认收缩；得分率表格新增变化趋势列；章节独立建议先给整体分析和整体建议，再给分项针对性建议；缺少数据科目补充更有价值的通用建议；CloudBase 后端部署包同步更新 |
| 4.1.6 | 2026-05-17 | 各科趋势深度分析与增强分析调试信息默认收缩；得分率表格新增变化趋势列；章节独立建议先给整体分析和整体建议，再给分项针对性建议；缺少数据科目补充更有价值的通用建议；CloudBase 后端部署包同步更新 |
| 4.1.5 | 2026-05-16 | 在得分率表格与章节独立建议之间插入该科目成绩变化趋势图，继续保持表格设计不变；版本记录同步升级 |
| 4.1.3 | 2026-05-16 | 继续收紧 AI 深入分析排版：删除顶部重复摘要内容，章节独立建议改为分项列表，将“下一步”和“下次目标”放到建议后面；增强分析调试信息补充模型名称与使用方式；版本记录同步升级 |
| 4.1.2 | 2026-05-16 | 调整 AI 深入分析子模块排版：删除顶部重复诊断内容，将“下一步”并入章节独立建议，将“下次目标”放在建议后面；保留得分率表格不变；版本记录同步升级 |
| 4.1.1 | 2026-05-16 | 迁移小程序已保存的旧默认模型配置，避免仍沿用 qwen-max-latest / qwen-vl-max-latest；版本记录同步升级 |
| 4.1.0 | 2026-05-16 | AI分析改为结构化上下文 + JSON mode：后端先压缩成绩、历史和材料摘要，再让模型只补简要结论；默认模型切换为更快的百炼模型，增强分析轮询间隔同步缩短；版本记录同步升级 |
| 4.0.0 | 2026-05-16 | 大版本升级：增强分析改为按输入分项和材料进行强制针对性分析，英语等学科可逐项输出得分率和对应建议；同日期成绩冲突、导出内容、版本展示和 CloudBase 包同步整理；版本记录同步升级 |
| 3.0.23 | 2026-05-16 | 修复英语分项解析对自然输入格式的识别，确保听力/阅读/完形/语法/写作/续写等分项都能进入深入分析；增强分析以规则层分项结果为准，避免泛化结论覆盖已输入材料；版本记录同步升级 |
| 3.0.22 | 2026-05-16 | 增强分析强制按已输入学科补全输出，导出内容补充章节独立建议；英语分项建议按听力、阅读、完形、语法、写作、续写展开；版本记录同步升级 |
| 3.0.21 | 2026-05-16 | 同日期成绩保存时增加覆盖/保留弹窗；增强分析自动补全所有已输入学科；删除执行计划和页面底部三个分数；说明文字与小标题字号同步调整；版本记录同步升级 |
| 3.0.20 | 2026-05-16 | 删除 AI 增强分析中的核心诊断重复模块；增强分析对已输入分项材料的科目改为按分项给出针对性分析和建议；各科趋势图新增横纵轴坐标值；版本记录同步升级 |
| 3.0.18 | 2026-05-16 | 增强分析页改为按学科卡片展示，新增内容/得分/总分/得分率表格；删除阶段目标和家长关注点重复内容；增强分析材料学科名做归一化处理，降低“明明已录入却提示缺少数据”的误判；版本记录同步升级 |
