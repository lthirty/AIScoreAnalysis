...
### 微信登录
- 支持微信小程序环境 `wx.login()` 获取 code
- H5 调试阶段提供 Rayna / Xulei 两个模拟微信账号
- 登录信息存储于 `localStorage` 的 `AIScoreAnalysis:user` 键

### 历史成绩记录
- 登录用户分析完成后自动保存到历史
- 存储键：`AIScoreAnalysis:examRecords`，按用户 ID 分组，最多保存 30 条记录
- 记录包含：日期、城市、年级、总分、各科成绩、最高分参考
- Rayna 和 Xulei 的历史成绩必须相互隔离，不能混用
...
| 版本 | 日期 | 修改内容 |
|------|------|----------|
| 1.7.4 | 2026-05-11 | 文档强调每次修改后必须同步到 GitHub；同步模拟微信账号与历史记录架构说明 |
| 1.7.3 | 2026-05-11 | 新增 Rayna / Xulei 两个模拟微信测试账号；历史成绩按账号独立保存；趋势分析对比当前账号最近一次考试 |
| 1.7.2 | 2026-05-11 | 同步 package、HTML 和应用版本号；重新构建 dist 产物 |
| 1.7.1 | 2026-05-11 | 新增考试日期选择器；历史记录包含examDate字段 |
| 1.7.0 | 2026-05-11 | 新增微信登录；新增历史成绩记录与趋势分析 |
